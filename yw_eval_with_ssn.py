...
from yw_svx import *
ssn_model = torch.nn.DataParallel(get_ssn_cityscape_model().cuda())
...
def inference_sliding(model, img, scales):
    ...
    for scale in scales:
        ...
        _mean = torch.FloatTensor(mean_std[0]).view(1, 3, 1, 1).cuda()
        _std = torch.FloatTensor(mean_std[1]).view(1, 3, 1, 1).cuda()
        lab_crops = convert_01_tensor_rgb2lab(input_crops * _std + _mean)
